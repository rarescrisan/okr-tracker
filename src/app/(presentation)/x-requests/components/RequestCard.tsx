...
function ArrowIcon() {
  return (
    <svg className="w-4 h-4 text-[#6B7394] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <Card padding="none" className="overflow-hidden">
      ...
      <div className="hidden md:flex items-start gap-4 px-5 py-4">
            ...
            {request.requesting_user && (
              <div className="flex items-center gap-1 pl-1">
                <Avatar
                  ...
                  size="xs"
                />
                <span className="text-xs text-[#A0A8C8]">{request.requesting_user.name}</span>
              </div>
            )}
            {request.requesting_project && (
              <span className="text-xs text-[#6B7394] pl-1 truncate max-w-[140px]">
            ...
            {request.target_user && (
              <div className="flex items-center gap-1 pl-1">
                <Avatar
                  ...
                  size="xs"
                />
                <span className="text-xs text-[#A0A8C8]">{request.target_user.name}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Description */}
        <p className="flex-1 text-sm text-white line-clamp-3 min-w-0">{request.description}</p>

        {/* Status + actions */}
        <div className="flex items-center gap-2 shrink-0">
          <div className="relative group">
            <Badge dot color={opt.color}>{opt.label}</Badge>
            <div className="absolute right-0 top-full mt-1 z-10 hidden group-hover:block bg-[#2A3152] rounded-md shadow-lg border border-white/[0.08] py-1 min-w-[130px]">
              {nextStatuses.map(s => (
                <button
                  key={s.value}
                  onClick={() => onStatusChange(request.id, s.value as XRequest['status'])}
                  className="w-full text-left px-3 py-1.5 text-xs text-white hover:bg-white/[0.06] flex items-center gap-2"
                >
          ...
          <button
            ...
            className="p-1.5 text-[#6B7394] hover:text-[#FF4D6A] hover:bg-[#FF4D6A]/[0.12] rounded transition-colors"
      ...
      <div className="md:hidden p-4 space-y-3">
        ...
        {(request.requesting_user || request.target_user) && (
          <div className="flex items-center gap-3 text-xs text-[#A0A8C8]">
            {request.requesting_user && (
